...
def caoscondition(args, start_token):
    ...
    return {"type": "Condition", "args": args, "start_token": start_token, "end_token": end_token }
...
def parse_condition(state):
    ...
    if ate_whitespace and state.tokens[state.p][0] == TOK_WORD and state.tokens[state.p][1] in ("and", "or"):
        ...
        return caoscondition([left, caosconditionkeyword(comparison), right, caosconditionkeyword(combiner)] + remainder["args"], startp)
    else:
